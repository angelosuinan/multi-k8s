...
kubectl apply -f k8s
kubectl set image deployments/server-deployment server=angelosuinan/multi-server:latest
kubectl set image deployments/client-deployment client=angelosuinan/multi-client:latest
kubectl set image deployments/worker-deployment worker=angelosuinan/multi-worker:latest